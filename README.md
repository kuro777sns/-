...
- **🌏 全ジャンル横断**：ジャンルを問わず、売れているnoteをまとめて探す枠。
  各ジャンルの代表キーワードを束ねて一気に取得します
- **件数の内訳**：条件に合うnote／購入状況を確認した数／🔥買われています／
  売れている割合／平均価格／最高価格を、結果の上にまとめて表示
- **価格の下限をワンタップ**：なし・¥500・¥980・¥1,980・¥2,980・¥4,980
- **ジャンルは2段構え**。まず大分類を選び、必要なら小分類でさらに絞ります
  - 上段：🔥いま人気 / 🌏全ジャンル / 💕恋愛 / 🔮占い・スピ / 💼ビジネス・お金 / 🌿くらし
  - 大分類のままでも検索できるので、細かく選ばなくても使えます
  - 大分類を選ぶと、その中の小分類（恋愛なら復縁・婚活・男性心理…）だけが下に出ます
  - 大分類・小分類とそのキーワードは `assets/genres.js` の
    `GENRE_GROUPS` / `GENRES` で変更できます
- **🎯 ジャンル一致のみ**：ジャンル名がタイトルか本文冒頭に出てこない記事を除きます。
  note の検索はゆるく、指定と関係ない記事も返ってくるための精度対策です
  （「ゆるく拾う」に切り替えれば従来どおり全部出ます。何件除いたかも表示します）
- **キーワード検索**（例：「副業のはじめかた」）
...
公開URLをブラウザで開くだけです。PCでもスマホでも同じものが動きます。

### アイコンとして置く

ホーム画面やデスクトップに置くと、ブラウザのタブや検索窓なしで開けます。

| 端末 | 手順 |
|---|---|
| iPhone / iPad | Safariで開く → 共有ボタン（□に↑）→ **ホーム画面に追加** |
| Android | Chromeで開く → 右上の ⋮ → **ホーム画面に追加** / **アプリをインストール** |
| Windows | Edge や Chrome で開く → アドレスバー右の **インストール** アイコン、<br>または ⋮ → **アプリ** → **このサイトをアプリとしてインストール** |
| Mac | Chrome の ⋮ → **キャストと共有** → **ページを別ウィンドウで開く**、<br>または Safari の共有 → **Dock に追加** |

`manifest.webmanifest` を置いてあるので、名前「noteさがし」と🍓アイコンで登録されます。

### 端末ごとの注意

**⚙️設定（CORSプロキシ）はブラウザごとに保存されます。**
PCで登録しても、スマホでは改めて登録が必要です。同じURLを入れてください。

お気に入り（⭐）も端末ごとに別々に保存されます。
...
⚙️設定にCORSプロキシを登録すると、アプリが**取得量を自動で引き上げます**。

| | 共用の中継 | 自前の中継 |
|---|---|---|
| キーワードごとの取得ページ数 | 4 | **12** |
| 購入状況を調べる上限 | 300件 | **900件** |

noteの検索結果は大半が無料noteなので、有料 × 価格 × 購入実績で絞ると
一気に減ります。母数を増やすことがそのままヒット数に効きます。
